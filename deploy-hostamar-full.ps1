...
if (-not $SkipVMPrep) {
    ...
    scp "$(Join-Path $BasePath 'hostamar-platform/deploy/vm-prepare.sh')" hostamar-iap:/tmp/
...
if (Test-Path $deployScript) {
    & $deployScript -SSHHost "hostamar-iap"
...
scp "$(Join-Path $BasePath 'hostamar-platform/deploy/nginx-config.sh')" hostamar-iap:/tmp/

Write-Host "  Configuring Nginx..." -ForegroundColor Gray
ssh hostamar-iap "chmod +x /tmp/nginx-config.sh && /tmp/nginx-config.sh $Domain"
...
if (-not $SkipSSL) {
    ...
    if ($proceed -eq 'y') {
        ...
        scp "$(Join-Path $BasePath 'hostamar-platform/deploy/setup-ssl.sh')" hostamar-iap:/tmp/
        
        Write-Host "  Obtaining SSL certificate..." -ForegroundColor Gray
        ssh hostamar-iap "chmod +x /tmp/setup-ssl.sh && /tmp/setup-ssl.sh $Domain"
...
ssh hostamar-iap "pm2 status"

Write-Host "`n  Testing health endpoint..." -ForegroundColor Gray
$health = ssh hostamar-iap "curl -s http://localhost:3001/api/health"
...
ssh hostamar-iap "systemctl list-timers hostamar-* --no-pager"
...
if (-not $SkipSSL) {
    ...
    Write-Host "  http://$(ssh hostamar-iap 'curl -s ifconfig.me 2>/dev/null')" -ForegroundColor Yellow
}

Write-Host "`n📝 Useful commands:" -ForegroundColor Cyan
Write-Host "  View logs:    ssh hostamar-iap 'pm2 logs hostamar-platform'" -ForegroundColor Gray
Write-Host "  Restart app:  ssh hostamar-iap 'pm2 restart hostamar-platform'" -ForegroundColor Gray
Write-Host "  Check status: ssh hostamar-iap 'pm2 status'" -ForegroundColor Gray
Write-Host "  Monitor:      ssh hostamar-iap 'pm2 monit'" -ForegroundColor Gray
